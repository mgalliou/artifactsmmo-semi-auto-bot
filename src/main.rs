...
fn run() {
    ...
    let mut char1 = Character::new(
        &account,
        &account.get_character_by_name("Jio").unwrap().name,
        bank.clone(),
    );
    let mut char2 = Character::new(
        &account,
        &account.get_character_by_name("Eraly").unwrap().name,
        bank.clone(),
    );
    let mut char3 = Character::new(
        &account,
        &account.get_character_by_name("Nalgisk").unwrap().name,
        bank.clone(),
    );
    let mut char4 = Character::new(
        &account,
        &account.get_character_by_name("Tieleja").unwrap().name,
        bank.clone(),
    );
    let mut char5 = Character::new(
        &account,
        &account.get_character_by_name("Kvarask").unwrap().name,
        bank.clone(),
    );
