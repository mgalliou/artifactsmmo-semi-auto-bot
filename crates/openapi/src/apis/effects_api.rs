...
pub async fn get_all_effects_effects_get(configuration: &configuration::Configuration, page: Option<u32>, size: Option<u32>) -> Result<models::StaticDataPageEffectSchema, Error<GetAllEffectsEffectsGetError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::StaticDataPageEffectSchema`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::StaticDataPageEffectSchema`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_effect_effects_code_get(configuration: &configuration::Configuration, code: &str) -> Result<models::EffectResponseSchema, Error<GetEffectEffectsCodeGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
