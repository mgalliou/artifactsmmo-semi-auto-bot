...
impl CharacterController {
    ...
    fn progress_task_reward_order(
        ...
        match self.can_exchange_tasks_coins() {
            ...
            Err(TasksCoinExchangeCommandError::MissingCoins(quantity)) => {
                self.order_board
                    .add(TASKS_COIN, quantity, None, order.purpose.clone())?;
                Ok(0)
            }
            Err(e) => Err(e.into()),
        }
    ...
    fn can_exchange_tasks_coins(&self) -> Result<(), TasksCoinExchangeCommandError> {
        ...
        let missing = if self.order_board.is_ordered(TASKS_COIN) {
            min
        } else {
            min.saturating_sub(available)
        };
        if missing > 0 {
    ...
    fn can_gather_now(&self, resource: &Resource) -> Result<(), GatherCommandError> {
    ...
    fn can_gather(&self, resource: &Resource) -> Result<(), GatherCommandError> {
    ...
    pub fn can_fight(&self, monster: &Monster) -> Result<(), KillMonsterCommandError> {
        if !self.skill_enabled(Skill::Combat) {
    ...
    pub fn can_craft_now(
        ...
        item: &str,
        quantity: u32,
    ) -> Result<(Item, Skill), CraftCommandError> {
    ...
    pub fn can_craft(&self, item: &str) -> Result<(Item, Skill), CraftCommandError> {
        ...
        if self.client.skill_level(skill) < item.level() {
            return Err(CraftCommandError::InsufficientSkillLevel(
                skill,
                item.level(),
            ));
        }
    ...
    fn eat_food_from_inventory(&self) {
        self.inventory
            ...
            .for_each(|food| {
                ...
                if self
                    .account
                    .time_to_get(food.code())
                    .is_some_and(|time| time * quantity < time_to_rest(self.missing_hp() as u32))
                {
                    quantity += 1;
                }
                if quantity > 0 {
                    quantity = min(quantity, self.inventory.total_of(food.code()));
                    if let Err(e) = self.use_item(food.code(), quantity) {
    ...
    fn can_buy_item(
        ...
        let missing_currency = if self.order_board.is_ordered(npc_item.currency()) {
            total_price
        } else {
            total_price.saturating_sub(available_currency)
        };
        ...
        if self.maps.with_content_code(npc_item.npc_code()).is_empty() {
    ...
    fn can_sell_item(
        ...
        item_code: &str,
        quantity: u32,
    ) -> Result<NpcItem, SellNpcCommandError> {
